...
namespace CMDR
{
    public struct Quaternion
    {
        ...
        public Vector4 ToAxisAngle()
        {
            ...
            result.X = Vec.X / MathF.Sin(theta / 2);
            result.Y = Vec.Y / MathF.Sin(theta / 2);
            result.Z = Vec.Z / MathF.Sin(theta / 2);
            result.W = theta;
        ...
        public static Quaternion CreateRotation(Vector3 axis, float angle)
        {
            ...
            axis *= MathF.Sin(angle);

            return new Quaternion() { W = MathF.Cos(angle), Vec = axis };
        }
        ...
        public static Quaternion FromEuler(float xRot, float yRot, float zRot)
        {
            ...
            float sinX = MathF.Sin(xRot);
            float sinY = MathF.Sin(yRot);
